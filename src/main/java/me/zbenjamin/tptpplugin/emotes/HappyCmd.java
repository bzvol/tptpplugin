    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length == 0){
            ...
            if (Bukkit.getServer().getPlayerExact(args[0]) != null) {
                if (Methods.getLocaleHu()) Bukkit.getServer().broadcastMessage(ChatColor.WHITE +
                        Objects.requireNonNull(Bukkit.getServer().getPlayer(args[0])).getDisplayName() + ChatColor.GREEN + ChatColor.BOLD + " boldog :)");
                else Bukkit.getServer().broadcastMessage(ChatColor.WHITE +
                        Objects.requireNonNull(Bukkit.getServer().getPlayer(args[0])).getDisplayName() + ChatColor.GREEN + ChatColor.BOLD + " is happy :)");
                return true;
            }
            else{
                if (sender instanceof Player) {
